...
> 上图中，定时器工作在递增计数模式，纵轴是计数器的计数值 CNT，横轴表示时间。当CNT < CCRx 时，IO 输出低电平（逻辑 0）；当 CNT >= CCRx 时，IO 输出高电平（逻辑 1）；当CNT = ARR 时，定时器溢出，CNT 的值被清零，然后继续递增，依次循环。在这个循环中，改变 CCRx 的值，就可以改变 PWM 的占空比，改变 ARR 的值，就可以改变 PWM 的频率，这就是 PWM 输出的原理。
...
**编程实战：PWM输出控制LED灯实现呼吸灯**
  ...
  > **占空比越高，LED越亮，而不是越暗。这是因为在PWM信号中，占空比（Duty Cycle）表示在一个周期内信号为高电平（ON）的时间比例。对于LED亮度控制，信号为高电平时LED是点亮的，因此高电平时间越长，LED就会显得越亮。**
- 初始化TIM
```c
